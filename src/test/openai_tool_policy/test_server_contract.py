...
def test_stream_usage(model):
    ...
    for path, base_payload in requests.items():
        for include_usage in (False, True):
            ...
            if status != 200 or not events or events[-1] != "[DONE]":
                raise AssertionError(f"invalid {path} SSE response: HTTP {status}: {events}")
            chunks = events[:-1]
            if include_usage:
                if len(chunks) < 3:
                    raise AssertionError(f"expected content, finish, and usage chunks for {path}: {chunks}")
                if any(chunk.get("usage", "missing") is not None for chunk in chunks[:-1]):
                    raise AssertionError(f"ordinary chunks must contain usage null for {path}: {chunks}")
                if chunks[-1].get("choices") != [] or not isinstance(chunks[-1].get("usage"), dict):
                    raise AssertionError(f"invalid final usage chunk for {path}: {chunks[-1]}")
            elif any("usage" in chunk for chunk in chunks):
                raise AssertionError(f"usage appeared without include_usage for {path}: {chunks}")
