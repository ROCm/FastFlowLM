...
struct Harness {
    ...
    std::unique_ptr<phi4_rai> engine;
    ...
    explicit Harness(std::function<void(fake_corelib::State&)> configure = {}) {
        ...
        engine = std::make_unique<phi4_rai>(LM_Config{}, package, runtime);
    }
    ~Harness() {
        engine.reset();
        package.reset();
...
void TestWeightCacheReplacesPackingOnTheSecondLoad() {
    ...
    {   // first load: packs, and leaves a cache behind
        ...
        auto engine = std::make_unique<phi4_rai>(LM_Config{}, package, runtime);
...
void TestStaleWeightCacheFallsBackToPacking() {
    ...
    const auto index_path = directory / "phi4-rai-weights.json";
    ...
    const auto index = flm::phi4::ReadWeightCacheIndex(
        directory, flm::phi4::MakeWeightCacheKey(
                       package->Path(), RYZENAI_CORELIB_VERSION_MAJOR,
                       RYZENAI_CORELIB_VERSION_MINOR,
                       RYZENAI_CORELIB_VERSION_PATCH, 64, 161));
...
void TestStaleWeightCacheIsReclaimed() {
    ...
    { std::ofstream out(directory / "phi4-rai-weights.json", std::ios::binary); out << "{}"; }
    ...
    TEST_REQUIRE(!std::filesystem::exists(directory / "phi4-rai-weights.json"));
...
void TestNoDeviceObjectExistsWhenPackageValidationFails() {
    ...
    RequireContains(RequireThrows([&] {
        phi4_rai engine(LM_Config{}, package, runtime);
    }), "blk.1");
...
void TestVProjectionWritesWindowAtPositionTimes128() {
    ...
    (void)h.engine->forward(1);
    // Since corelib 0.5.0 the engine also opens 2-D views of its operands, to
    // bind each at the padded extent its op was planned for. A cache write is
    // the only 3-D window, so select on that rather than assuming every window
    // is a V write -- what this test is about is where the V projection lands,
    // not how many views the dispatch path needs.
    std::vector<fake_corelib::TensorWindowRecord> cache_windows;
    for (const auto& window : fake_corelib::GetState().tensor_windows) {
        if (window.shape.size() == 3) cache_windows.push_back(window);
    }
    TEST_REQUIRE(cache_windows.size() == 32);
    TEST_REQUIRE(cache_windows.front().shape ==
                 std::vector<std::int64_t>({8, 4089, 128}));
    TEST_REQUIRE(cache_windows.front().offset == 7 * 128);
...
void TestTwoConcurrentRaiRequestsNeverOverlapDispatch() {
    Harness h;
    auto second_engine = std::make_unique<phi4_rai>(
...
#define RUN_TEST(name) RunTest(&name, #name)
    ...
    RUN_TEST(TestTwoConcurrentRaiRequestsNeverOverlapDispatch);
