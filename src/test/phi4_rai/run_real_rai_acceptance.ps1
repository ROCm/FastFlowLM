...
$modelDir=Join-Path $env:USERPROFILE '.flm/models/phi4-mini-it-rai'
# Inert when flm.exe was built with FLM_ENABLE_RAI (corelib is linked
# in there); still used by the DLL-loading test binaries in this directory.
$env:FLM_RAI_CORELIB_PATH=$core
...
try{
 ...
 $record.weight_cache_present_at_start=@($cacheNames|Where-Object{Test-Path (Join-Path $modelDir $_)})
 $py=@'
...
'@
 ...
 try{for($i=0;$i-lt 300;$i++){try{$v=Invoke-RestMethod "http://127.0.0.1:$Port/api/version";break}catch{Start-Sleep -Milliseconds 200}};if(-not$v){throw 'server not ready'}
  Mark 'server ready'
  # corelib is import-linked, not dlopened, so flm never prints the DLL path it
  # ended up with and the transcript cannot prove which one ran. The loaded
  # module list of the live process can, and it is the only thing that can.
  $server.Refresh();$loaded=@($server.Modules|Where-Object{$_.ModuleName-eq 'ryzenai_corelib.dll'}|% {$_.FileName})
  $record.corelib_loaded_by_server=$loaded
  if($loaded.Count-ne 1){throw "server loaded $($loaded.Count) copies of ryzenai_corelib.dll"}
  if((Resolve-Path $loaded[0]).Path-ne $core){throw "server loaded $($loaded[0]), not $core"}
  Mark 'corelib module proof ok'
