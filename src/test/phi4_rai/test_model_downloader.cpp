...
void TestActualRaiCatalogTreatsPinnedConfigWithoutFlmVersionAsCompatible() {
    ...
    TEST_REQUIRE(downloader.is_model_downloaded(kRaiTag, true, true) ==
...
void TestModelInfoHasExactSizeAndSha256ForEveryRequiredFile() {
    ...
    const auto& records = all_info.at(kRaiModelInfoKey);
    ...
    DownloaderFixture()
        ...
        const nlohmann::json catalog = {
            ...
            {"models", {{"test-model", {{"1b", {
                ...
                {"details", {{"family", "phi4"}}},
                // Present but empty: this is what marks the entry as
                // pinned-upstream, and the per-file lookup is expected to fail
                // on it rather than the entry silently going unverified.
...
int main() {
    RunTest(TestRaiCatalogHasExactlyFourFilesAndExpectedDirectoryName, "rai catalog");
    ...
    RunTest(TestActualRaiCatalogTreatsPinnedConfigWithoutFlmVersionAsCompatible,
