...
inline std::pair<std::string, std::unique_ptr<AutoModel>> get_qwen3vl_model(const std::string& model_tag) {
    static std::unordered_set<std::string> qwen3vl_flash_Tags = {
        "qwen3vl-flash", "qwen3vl-flash:4b"
    };

    std::unique_ptr<AutoModel> auto_chat_engine = nullptr;
    std::string new_model_tag = model_tag;
    if (qwen3vl_flash_Tags.count(model_tag))
        new_model_tag = "qwen3vl-flash:4b";
    else {
        header_print("warn", "Unknown tag '" << model_tag << "', falling back to qwen3vl-flash:4b");
        new_model_tag = "qwen3vl-flash:4b";
    }
    auto_chat_engine = std::make_unique<Qwen3VL_Flash>(&npu_device_global);
...
int main(int argc, char* argv[]) {
    ...
    std::unique_ptr<AutoModel> chat = std::make_unique<Qwen3VL_Flash>(&npu_device_global);
