...
#include <models/phi4/aie4/phi4_aie4_gguf.hpp>
#endif
#include "server.hpp"
...
std::unique_ptr<flm::backend::ModelBackend> MakeAie2pStub(
    const flm::backend::BackendContext& context) {
    ++g_factory.legacy_calls;
    return std::make_unique<StubBackend>(std::string(flm::backend::kAie2pBackendId),
...
std::unique_ptr<flm::backend::ModelBackend> MakeAie4Stub(
    const flm::backend::BackendContext& context) {
    ...
    auto backend = std::make_unique<StubBackend>(
        std::string(flm::backend::kAie4BackendId), context.context_length);
    backend->decode_limit_ = flm::phi4::kAie4DecodeLimit;
...
/// \note It names no backend. It cannot: the backend is the hardware, and the
///       catalog entry has already been filtered for that hardware by the time
///       it reaches here. The tests say which backend they mean by passing it
///       to Load, which is the --backend path.
...
/// \note A backend id is a hardware id. These match utils::platform_id, and the
///       shared constants in model_backend.hpp; they are spelled out here so the
///       suite reads as "run this on that silicon".
constexpr const char* kAie2p = "aie2p";
constexpr const char* kAie4 = "aie4";
...
struct FactoryScope {
    FactoryScope() {
        ...
        registry.replace_backend("phi4", kAie2p, MakeAie2pStub);
#if defined(FLM_ENABLE_AIE4)
        registry.replace_backend("phi4", kAie4, MakeAie4Stub,
...
bool OnAie4(const AutoModel& model) {
    return model.backend_id() == kAie4;
}
bool OnAie2p(const AutoModel& model) { return model.backend_id() == kAie2p; }
...
std::unique_ptr<Phi4> Load(const TempPackage& package,
                           ...
                           const std::string& backend = kAie2p) {
...
void TestCorelibAie4GgufBuildsOnlyTheCorelibEngine() {
    TempPackage package;
    FactoryScope scope;
    auto model = Load(package, ModelInfo(), -1, false, nullptr, kAie4);
    TEST_REQUIRE(g_factory.legacy_calls == 0);
    TEST_REQUIRE(g_factory.aie4_calls == 1);
    TEST_REQUIRE(OnAie4(*model));
    TEST_REQUIRE(!Phi4FrontendTestAccess::HasLegacyNpu(*model));
}
...
void TestAie4ProfileUsesCachedRuntimeDllPathAfterEnvironmentChanges() {
    ...
    auto aie4 = Load(package, ModelInfo(), -1, false, nullptr, kAie4);
    g_factory.runtime.reset();
    const auto aie4_profile = aie4->show_profile();
    RequireContains(aie4_profile, kAie4);
    ...
    TEST_REQUIRE(legacy_profile.find(kAie4) == std::string::npos);
...
void TestFeatureOffRejectsAie4TagWithoutIncludingCorelibHeaders() {
    ...
    RequireContains(RequireThrows([&] {
        (void)Load(package, ModelInfo(), -1, false, nullptr, kAie4);
...
void TestInvalidPackageFailsBeforeRuntimeAndDeviceCreation() {
    ...
    FactoryScope scope;
    RequireThrows([&] { (void)Load(package, ModelInfo(), -1, false, nullptr, kAie4); });
    TEST_REQUIRE(g_factory.aie4_calls == 0);
}
...
void TestMissingCorelibFailsOnlyWhenAie4ModelLoads() {
    ...
    FactoryScope scope;
    g_factory.throw_for_aie4 = true;
    RequireContains(RequireThrows([&] { (void)Load(package, ModelInfo(), -1, false, nullptr, kAie4); }), "missing corelib");
    TEST_REQUIRE(g_factory.legacy_calls == 0);
}
...
void TestAie4SelectionWithMissingDllFailsWithoutChangingBackend() {
    ...
    g_factory.throw_for_aie4 = true;
    RequireContains(RequireThrows([&] {
        model.load_model(package.path().string(), ModelInfo(), -1, false, kAie4);
    }), "missing corelib");
...
void TestOrdinaryModelLoadsAfterAnAie4RuntimeLoadFailure() {
    ...
    TEST_REQUIRE(g_factory.legacy_calls == 1);
    TEST_REQUIRE(OnAie2p(*ordinary));
}

void TestPreemptionIsRejectedForTheAie4Route() {
    TempPackage package;
    FactoryScope scope;
    RequireContains(RequireThrows([&] { (void)Load(package, ModelInfo(), -1, true, nullptr, kAie4); }), "preemption");
    TEST_REQUIRE(g_factory.aie4_calls == 0);
}

std::unique_ptr<Phi4> ReadyAie4(const TempPackage& package) {
    return Load(package, ModelInfo(), -1, false, nullptr, kAie4);
}
