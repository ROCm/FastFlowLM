...
struct Harness {
    ...
    std::unique_ptr<phi4_aie4> engine;
    ...
    explicit Harness(std::function<void(fake_corelib::State&)> configure = {}) {
        ...
        engine = std::make_unique<phi4_aie4>(LM_Config{}, package, runtime);
    }
    ~Harness() {
        engine.reset();
        package.reset();
...
void TestWeightCacheReplacesPackingOnTheSecondLoad() {
    ...
    {   // first load: packs, and leaves a cache behind
        ...
        auto engine = std::make_unique<phi4_aie4>(LM_Config{}, package, runtime);
...
void TestNoDeviceObjectExistsWhenPackageValidationFails() {
    ...
    RequireContains(RequireThrows([&] {
        phi4_aie4 engine(LM_Config{}, package, runtime);
    }), "blk.1");
...
void TestTwoConcurrentAie4RequestsNeverOverlapDispatch() {
    Harness h;
    auto second_engine = std::make_unique<phi4_aie4>(
