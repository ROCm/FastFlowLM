...
/// \note A backend id is *not* a platform id. Which generation a build targets
///       is utils::npu_platform's job; the two axes line up one-to-one today,
///       which is exactly why they need separate names -- otherwise the day
///       they stop lining up is the day the strings start lying.
...
/// \note Every model family registers this one; it is what a build without
///       FLM_ENABLE_RAI runs, on whatever generation it was built for.
...
///       have a corelib engine.
inline constexpr const char* kRaiBackendId = "rai";

/// \brief the backend a build falls back to when nothing else picks one
/// \return kRaiBackendId when built with FLM_ENABLE_RAI, kFlmBackendId otherwise
/// \note A build links one kernel flow or the other, never both, so this is a
///       compile-time fact. It is deliberately *not* derived from
///       utils::platform_id: the backend axis and the platform axis are allowed
///       to disagree, and routing one through the other is what used to make
///       them look like the same thing.
constexpr const char* build_default_backend_id() {
#ifdef FLM_ENABLE_RAI
    return kRaiBackendId;
#else
    return kFlmBackendId;
#endif
}
...
/// \param fallback the backend id to use when nothing else picks one, normally
///        build_default_backend_id()
...
/// \note Precedence: --backend, then FLM_BACKEND, then `fallback`. The catalog
///       does not name a backend: model_list has already pruned itself to the
///       entries this build's platform can run, and a per-entry backend list
///       would only restate what the build already links.
