/// \file aie2p_backend.hpp
/// \brief The aie2p backend, shared by every family that ships one
/// \note aie2p runs FastFlowLM's own NPU kernels, and every one of those
///       engines is built the same way -- construct, load the Q4NX weights,
///       clear the context -- so one template covers them all.
#pragma once
...
/// \brief the aie2p backend for a FastFlowLM NPU engine
...
class Aie2pBackend final : public ModelBackend {
public:
    explicit Aie2pBackend(const BackendContext& context) {
        if (!context.config) {
            throw std::runtime_error("aie2p backend needs an LM_Config");
        }
        if (!context.npu) {
            throw std::runtime_error("aie2p backend needs an NPU instance");
        }
    ...
    std::string id() const override { return kAie2pBackendId; }

private:
    std::unique_ptr<Engine> engine_;
};

/// \brief a factory building Aie2pBackend<Engine>
/// \tparam Engine the concrete engine type
/// \return a factory suitable for BackendRegistry::register_backend
template <class Engine>
BackendFactory aie2p_factory() {
    return [](const BackendContext& context) -> std::unique_ptr<ModelBackend> {
        return std::make_unique<Aie2pBackend<Engine>>(context);
    };
}
