"""Write a dequantized weight sidecar next to model.q4nx.
    ...
    python3 dequantize.py <model_dir> --mode bfp  --only "mlp."

Selects the projection tensors whose names contain --only, dequantizes each on
the CPU and writes them in the layout that mode's consumer reads. The container
is the same safetensors format as model.q4nx -- an 8-byte little-endian header
length, a JSON header, then blobs -- so a sidecar could later be merged into it
by concatenating tensor entries.

``bf16`` writes what dequant.xclbin would have written, in the same buffer
layout, which is what the shipped mm reads. ``bfp`` writes the bfp16ebs8 form
flm.GEMM takes; it calls the operator's own packer, so set IRON_PATH to an IRON
checkout. ``bf16`` needs only numpy.

Three facts the output rests on, each established against the shipped
dequant.xclbin: the value is ``min + scale * q`` in f32, the conversion to bf16
...
def main():
    ...
    ap.add_argument("--mode", default="bf16", choices=sorted(SUFFIX))
    ...
    header, data_start = read_header(q4nx)
    ...
    for layer_idx in range(cfg["num_hidden_layers"]):
        ...
        for suffix, which in IN_FEATURES.items():
            ...
            n_values = raw_bytes // BLOCK_BYTES * WEIGHTS_PER_BLOCK
            n_bytes = n_values * 2 if args.mode == "bf16" else n_values // 8 * 9
            out_name = name + SUFFIX[args.mode]

            def make(name=name, cols=cols):
                raw, _ = read_tensor(q4nx, header, data_start, name)
                payload, _ = build(raw, cols, args.mode)
                return payload

            plan.append((out_name, n_bytes, DTYPE[args.mode], make))
    ...
    note = {
        "producer": "make_py",
        "mode": args.mode,
        "layout": "dequant.xclbin output order; see dequant_offset",
        "source": "model.q4nx",
    }
    for i, name in enumerate(write_container(out_path, plan, note)):
