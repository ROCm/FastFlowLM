...
/// to tell that from disk I/O or from shape planning. Set FLM_RAI_PROFILE_LOAD
...
struct phi4_rai::Impl {
    ...
    Impl(LM_Config, std::shared_ptr<Phi4GgufPackage> pkg,
        ...
        // The shape plan used to be built here, in the initializer list. Since
        // 0.5.0 the padding helpers take the stream, so the plan cannot exist
        // before one does and is built further down instead.
        ...
        api->Check(api->functions().create_stream(kPrefillPdi,kTokenPdi,&raw),"ryzenai_corelib_create_stream"); stream=UniqueStream(api,raw);
        // Now that the stream exists the padding helpers can be asked. Every
        // extent below is therefore an extent *for this stream's PDI pair*.
        plan=Phi4ShapePlan::Build(api,stream.get());
    ...
    buffer<bf16> run(std::span<const int> ids,bool prefill){
        ...
        try{
            ...
            // 0.5.0 took the row count out of every dispatch: M now comes from
            // the operand's own shape, and each operand is checked against its
            // OWN padded extent rather than resolved through the allocation it
            // sits in. So each op sees the tensors through a window at the
            // extent the plan interrogated for it. Built once here, not per
            // layer: ids.size() does not change inside the loop, and the
            // allocations themselves still happen only at load.
            const auto& extents=plan.ForRows(ids.size());
            auto view=[&](void* parent,std::int64_t rows,std::int64_t cols,const char* label){
                std::array<std::int64_t,2> shape{rows,cols};void* p=nullptr;
                api->Check(api->functions().create_tensor_window(parent,shape.data(),shape.size(),0,&p),std::string("ryzenai_corelib_create_tensor_window ")+label);
                return UniqueTensorWindow(api,p);
            };
            const auto hidden_q=view(hidden.get(),extents.query_rows,kHiddenSize,"hidden@query");
            const auto hidden_kv=view(hidden.get(),extents.kv_rows,kHiddenSize,"hidden@kv");
            const auto hidden_out=view(hidden.get(),extents.output_rows,kHiddenSize,"hidden@output");
            const auto hidden_mlp=view(hidden.get(),extents.ssmlp_rows,kHiddenSize,"hidden@ssmlp");
            const auto q_mm=view(q.get(),extents.query_rows,kQueryDimension,"query@matmul");
            const auto q_mha=view(q.get(),extents.flat_mha_rows,kQueryDimension,"query@mha");
            const auto k_mm=view(k.get(),extents.kv_rows,kKvDimension,"key@matmul");
            const auto k_mha=view(k.get(),extents.flat_mha_rows,kKvDimension,"key@mha");
            const auto attention_mha=view(attention.get(),extents.flat_mha_rows,kQueryDimension,"attention@mha");
            const auto attention_mm=view(attention.get(),extents.output_rows,kQueryDimension,"attention@matmul");
            const auto residual_mlp=view(residual.get(),extents.ssmlp_rows,kHiddenSize,"residual@ssmlp");
            const auto skip_mlp=view(skip.get(),extents.ssmlp_rows,kHiddenSize,"skip@ssmlp");
            void* res=residual_mlp.get();void* sk=skip_mlp.get();
            for(std::size_t i=0;i<kLayerCount;++i){
                const auto query_status=api->functions().matmul(
                    stream.get(),hidden_q.get(),q_weights[i].get(),q_mm.get());
                ...
                api->Check(api->functions().matmul(stream.get(),hidden_kv.get(),k_weights[i].get(),k_mm.get()),"ryzenai_corelib_matmul_bf16 key layer "+std::to_string(i));
                // The gm exception: a cache write takes its position from the
                // bound buffer's base, so this window stays a position, not an
                // extent -- its height is what bounds the rows that land there.
            ...
            // lm_input and logits are allocated one row tall, which is exactly
            // how 0.5.0 is told to select the token PDI for the head.
            api->Check(api->functions().matmul(stream.get(),lm_input.get(),lm_weights.get(),logits.get()),"ryzenai_corelib_matmul_bf16 LM head");
...
phi4_rai::phi4_rai(LM_Config c,std::shared_ptr<Phi4GgufPackage> p,std::shared_ptr<CorelibRuntime> r,std::uint32_t m):impl_(std::make_unique<Impl>(std::move(c),std::move(p),std::move(r),m)){}
phi4_rai::~phi4_rai()=default;
buffer<bf16> phi4_rai::forward(int id){return impl_->run(std::span<const int>(&id,1),false);}
buffer<bf16> phi4_rai::prefill(std::vector<int>&ids,void*){return impl_->run(ids,true);}
void phi4_rai::set_context_length(int n){impl_->usable();if(n<0||static_cast<std::uint32_t>(n)>impl_->max_length)throw std::out_of_range("Phi-4 context length is out of range");impl_->position=n;}
...
void phi4_rai::load_weights(Q4NX&){impl_->usable();throw std::runtime_error("Phi-4 rai weights are loaded only from GGUF");}
void phi4_rai::update_max_length(std::uint32_t n){impl_->usable();if(!n||n>kMaxSequenceLength||n<static_cast<std::uint32_t>(impl_->position))throw std::out_of_range("Phi-4 maximum length is invalid");impl_->max_length=n;}
void phi4_rai::clear_context(){impl_->usable();impl_->position=0;impl_->saved.reset();}
buffer<bf16> phi4_rai::get_k_cache(int l,int i){return impl_->read_cache(true,l,i);}
buffer<bf16> phi4_rai::get_v_cache(int l,int i){return impl_->read_cache(false,l,i);}
int phi4_rai::get_current_context_length(){impl_->usable();return impl_->position;}
int phi4_rai::checkpoint(){impl_->usable();impl_->saved=impl_->position;return impl_->position;}
int phi4_rai::restore(){impl_->usable();if(!impl_->saved)return -1;return impl_->position=*impl_->saved;}
bool phi4_rai::poisoned()const noexcept{return impl_&&impl_->poisoned;}
} // namespace flm::phi4
