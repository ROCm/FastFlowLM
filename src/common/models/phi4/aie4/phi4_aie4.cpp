#include "models/phi4/aie4/phi4_aie4.hpp"
#include "aie4/corelib_object.hpp"
#include "models/phi4/aie4/phi4_aie4_constants.hpp"
#include "models/phi4/aie4/phi4_aie4_host.hpp"
#include "models/phi4/aie4/phi4_aie4_shape_plan.hpp"
#include "models/phi4/aie4/phi4_aie4_weight_cache.hpp"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdlib>
...
phi4_aie4::phi4_aie4(LM_Config c,std::shared_ptr<Phi4GgufPackage> p,std::shared_ptr<CorelibRuntime> r,std::uint32_t m):impl_(std::make_unique<Impl>(std::move(c),std::move(p),std::move(r),m)){}
phi4_aie4::~phi4_aie4()=default;
buffer<bf16> phi4_aie4::forward(int id){return impl_->run(std::span<const int>(&id,1),false);}
buffer<bf16> phi4_aie4::prefill(std::vector<int>&ids,void*){return impl_->run(ids,true);}
void phi4_aie4::set_context_length(int n){impl_->usable();if(n<0||static_cast<std::uint32_t>(n)>impl_->max_length)throw std::out_of_range("Phi-4 context length is out of range");impl_->position=n;}
...
void phi4_aie4::load_weights(Q4NX&){impl_->usable();throw std::runtime_error("Phi-4 AIE4 weights are loaded only from GGUF");}
void phi4_aie4::update_max_length(std::uint32_t n){impl_->usable();if(!n||n>kMaxSequenceLength||n<static_cast<std::uint32_t>(impl_->position))throw std::out_of_range("Phi-4 maximum length is invalid");impl_->max_length=n;}
void phi4_aie4::clear_context(){impl_->usable();impl_->position=0;impl_->saved.reset();}
buffer<bf16> phi4_aie4::get_k_cache(int l,int i){return impl_->read_cache(true,l,i);}
buffer<bf16> phi4_aie4::get_v_cache(int l,int i){return impl_->read_cache(false,l,i);}
int phi4_aie4::get_current_context_length(){impl_->usable();return impl_->position;}
int phi4_aie4::checkpoint(){impl_->usable();impl_->saved=impl_->position;return impl_->position;}
int phi4_aie4::restore(){impl_->usable();if(!impl_->saved)return -1;return impl_->position=*impl_->saved;}
bool phi4_aie4::poisoned()const noexcept{return impl_&&impl_->poisoned;}
} // namespace flm::phi4
