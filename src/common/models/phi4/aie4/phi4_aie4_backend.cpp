...
constexpr const char* kAie4Gguf = "Phi-4-mini-instruct.Q8_0.gguf";
...
const std::vector<int> kAie4EosIds = {200020, 199999};
...
/// \brief Phi-4 on ryzenai-corelib, AIE4, GGUF weights
class CorelibAie4Backend final : public flm::backend::ModelBackend {
public:
    explicit CorelibAie4Backend(const flm::backend::BackendContext& context) {
        if (!context.config) {
            throw std::runtime_error("Phi-4 AIE4 backend needs an LM_Config");
        }
        if (context.enable_preemption) {
            throw std::invalid_argument("Phi-4 AIE4 does not support preemption");
        }
        if (context.context_length < 1 ||
            context.context_length > kAie4ContextLimit) {
        ...
        auto engine = std::make_unique<phi4_aie4>(
    ...
    std::string id() const override { return flm::backend::kAie4BackendId; }
    ...
    std::uint32_t max_decode_length() const override { return kAie4DecodeLimit; }
...
private:
    ...
    std::unique_ptr<phi4_aie4> engine_;
};
...
flm::backend::BackendFactory aie4_factory() {
    return [](const flm::backend::BackendContext& context)
        ...
        return std::make_unique<CorelibAie4Backend>(context);
    };
}
