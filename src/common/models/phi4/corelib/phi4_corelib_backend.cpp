...
public:
    explicit CorelibAie4Backend(const flm::backend::BackendContext& context) {
        ...
        if (context.context_length < 1 ||
            context.context_length > kCorelibAie4ContextLimit) {
        ...
        auto engine = std::make_unique<phi4_corelib_aie4>(
    ...
    std::string id() const override { return kCorelibAie4BackendId; }
    ...
    std::uint32_t max_decode_length() const override { return kCorelibAie4DecodeLimit; }
...
private:
    ...
    std::unique_ptr<phi4_corelib_aie4> engine_;
};
